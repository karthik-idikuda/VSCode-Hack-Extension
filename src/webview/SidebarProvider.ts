...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    private _issues: any[] = [];
    private _analyzers: AnalyzerInstance;
    ...
    public updateResults(issues: any[]) {
        this._issues = issues;
        if (this._view) {
    ...
    public getCurrentIssues(): any[] {
        return this._issues;
    }

    private formatIssues(issues: any[]) {
    ...
    private async fixSingleIssue(index: number) {
    ...
    private async explainIssue(index: number) {
        const issue = this._issues[index];
        if (!issue) return;
