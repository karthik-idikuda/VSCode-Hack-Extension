...
export class GeminiClient {
    ...
    async analyze(code: string, language: string, issueType: string): Promise<string> {
        if (!this.apiKey) {
    ...
    async generateFix(code: string, issue: any): Promise<string | null> {
        if (!this.apiKey) {
    ...
    async explainIssue(issue: any): Promise<string> {
        if (!this.apiKey) {
...
Keep responses concise but helpful.
    ...
    private buildAnalysisPrompt(code: string, language: string, issueType: string): string {
        return `
...
Respond in a friendly, conversational tone.
    ...
    private buildFixPrompt(code: string, issue: any): string {
        return `
...
Provide ONLY the fixed code, no explanations. Make the minimal change necessary to fix the issue.
    ...
    private getOfflineAnalysis(code: string, language: string, issueType: string): string {
